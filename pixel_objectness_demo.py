...
def draw_feature1(image_bgr):
    ...
    input_blob = image.transpose(2, 0, 1).reshape((1, ch, IMAGE_SIZE, IMAGE_SIZE))
    output = net.forward_all(**{net.inputs[0]: input_blob})
    conv1 = np.sum(net.blobs["conv1_2"].data[0],0)
    rows = image_bgr.shape[0]
    cols = image_bgr.shape[1]
    conv1 = conv1[0:rows, 0:cols]
    conv1 = Normalize(conv1)
    conv1 = 255*conv1
    feature1 = conv1.astype(np.uint8)
    return feature1


def conv1(input_image):
    input_image = Image.open(BytesIO(input_image))
    image_rgb = np.asarray(input_image.convert('RGB'), dtype=np.uint8)
    # OpenCV uses BGR color
    image = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2BGR)
    ...
    if result is None:
        ...
        out_image = draw_feature1(image)
    image_nd = cv2.imencode('.jpg', out_image)[1]
    return image_nd.tostring()   


def draw_feature2(image_bgr):
    mean = np.array([[[104.008, 116.669, 122.675]]], dtype=np.float32)
    image = image_bgr - mean
    ...
    input_blob = image.transpose(2, 0, 1).reshape((1, ch, IMAGE_SIZE, IMAGE_SIZE))
    output = net.forward_all(**{net.inputs[0]: input_blob})
    conv = np.sum(net.blobs["conv2_2"].data[0],0)
    rows = image_bgr.shape[0]//2
    cols = image_bgr.shape[1]//2
    conv = conv[0:rows, 0:cols]
    conv = Normalize(conv)
    conv = 255*conv
    feature = conv.astype(np.uint8)
    return feature


def conv2(input_image):
    input_image = Image.open(BytesIO(input_image))
    image_rgb = np.asarray(input_image.convert('RGB'), dtype=np.uint8)
    # OpenCV uses BGR color
    image = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2BGR)
    ...
    if result is None:
        ...
        out_image = draw_feature2(image)
    image_nd = cv2.imencode('.jpg', out_image)[1]
    return image_nd.tostring()   


def draw_feature3(image_bgr):
    mean = np.array([[[104.008, 116.669, 122.675]]], dtype=np.float32)
    image = image_bgr - mean
    ...
    input_blob = image.transpose(2, 0, 1).reshape((1, ch, IMAGE_SIZE, IMAGE_SIZE))
    output = net.forward_all(**{net.inputs[0]: input_blob})
    conv = np.sum(net.blobs["conv3_3"].data[0],0)
    rows = image_bgr.shape[0]//4
    cols = image_bgr.shape[1]//4
    conv = conv[0:rows, 0:cols]
    conv = Normalize(conv)
    conv = 255*conv
    feature = conv.astype(np.uint8)
...
def conv3(input_image):
    ...
    if result is None:
        ...
        out_image = draw_feature3(image)
    image_nd = cv2.imencode('.jpg', out_image)[1]
    return image_nd.tostring()   


def draw_feature4(image_bgr):
    mean = np.array([[[104.008, 116.669, 122.675]]], dtype=np.float32)
    image = image_bgr - mean
    ...
    input_blob = image.transpose(2, 0, 1).reshape((1, ch, IMAGE_SIZE, IMAGE_SIZE))
    output = net.forward_all(**{net.inputs[0]: input_blob})
    conv = np.sum(net.blobs["pool4"].data[0],0)
    rows = image_bgr.shape[0]//8
    cols = image_bgr.shape[1]//8
    conv = conv[0:rows, 0:cols]
    conv = Normalize(conv)
    conv = 255*conv
    feature = conv.astype(np.uint8)
...
def conv4(input_image):
    ...
    if result is None:
        ...
        out_image = draw_feature4(image)
    image_nd = cv2.imencode('.jpg', out_image)[1]
    return image_nd.tostring()   

def draw_feature5(image_bgr):
    mean = np.array([[[104.008, 116.669, 122.675]]], dtype=np.float32)
    image = image_bgr - mean
    ...
    input_blob = image.transpose(2, 0, 1).reshape((1, ch, IMAGE_SIZE, IMAGE_SIZE))
    output = net.forward_all(**{net.inputs[0]: input_blob})
    conv = np.sum(net.blobs["pool5"].data[0],0)
    rows = image_bgr.shape[0]//8
    cols = image_bgr.shape[1]//8
    conv = conv[0:rows, 0:cols]
    conv = Normalize(conv)
...
net = init_net()
server.serve(segment_image, conv1, conv2, conv3, conv4, conv5, port=os.environ.get('PORT'))
